...
export const useEvaluationManager = () => {
  ...
  // Initialize active tasks by loading all existing tasks
  const initializeActiveTasks = useCallback(async () => {
    try {
      console.log("Initializing active tasks...");

      // Get all RAG tasks
      const ragTasksResponse = await services.rag.task.getTasks();
      const ragTasks = ragTasksResponse.tasks || [];
      console.log("Found RAG tasks:", ragTasks);

      // Get all Prompt tasks
      const promptTasks = await services.prompt.task.getAllTasks();
      console.log("Found Prompt tasks:", promptTasks);

      // Update activeTasks with all existing task IDs
      const newActiveTasks = {
        rag: ragTasks.map((task: TaskDto) => task.id),
        prompt: promptTasks.map((task: PromptTask) => task.taskId),
      };
  ...
  }, [services.rag.task, services.prompt.task, taskCleanup]);
