...
const PromptEvaluationDetailPage: React.FC = () => {
  ...
  const { taskId, evaluationId } = useParams<{ taskId: string; evaluationId?: string }>();

  const [task, setTask] = useState<PromptTaskResponse | null>(null);
  const [evaluation, setEvaluation] = useState<PromptEvaluationResponse | null>(null);
  const [evaluationHistory, setEvaluationHistory] = useState<PromptEvaluationResponse[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [showNewEvaluationDialog, setShowNewEvaluationDialog] = useState(false);  const [newPrompt, setNewPrompt] = useState('');
  ...
  const loadEvaluationHistory = useCallback(async () => {
    ...
    try {
      const response = await promptTaskService.getTaskEvaluations(parseInt(taskId));      if ('detail' in response) {
  ...
  const prepareChartData = () => {
    ...
    return {
      ...
      datasets: [        {
          label: 'Prompt 评分',
          data: evaluationHistory.map(evaluation => parseInt(evaluation.promptScore)),
        ...
        {
          label: '语义相似度',
          data: evaluationHistory.map(evaluation => evaluation.semanticSimilarity),
        ...
        {
          label: '词汇多样性',
          data: evaluationHistory.map(evaluation => evaluation.lexicalDiversity),
      ...
      <Box sx={{ flex: 1, p: 3, overflow: 'auto' }}>
        <Box sx={{ display: 'flex', gap: 3, flexDirection: { xs: 'column', md: 'row' } }}>
          ...
          <Box sx={{ flex: 1 }}>
            ...
            {evaluation && (
              <Card sx={{ mb: 3 }}>
                <CardContent>                  <Typography variant="h6" sx={{ mb: 2, display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Box sx={{ width: { xs: '100%', md: '400px' }, flexShrink: 0 }}>
            <Card>
              <CardContent>
                ...
                <List sx={{ maxHeight: 400, overflow: 'auto' }}>                  {evaluationHistory.map((evaluation, index) => (
                    <ListItem
                      key={evaluation.evalId}
                      sx={{
                        border: 1,
                        borderColor: evaluation.evalId.toString() === evaluationId ? 'secondary.main' : 'divider',
                        borderRadius: 1,
                        mb: 1,
                        cursor: 'pointer',
                        '&:hover': {
                      ...
                      onClick={() => navigate(`/evaluation/prompt/${taskId}/eval/${evaluation.evalId}`)}
                    >
                      <ListItemText
                        primary={`评估 ${index + 1}`}
                        secondary={
